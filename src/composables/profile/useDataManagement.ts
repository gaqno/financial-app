...
export function useDataManagement() {
  ...
  const exportAllData = async (): Promise<boolean> => {
    try {
      ...
      const fileContent = exportOptions.value.format === 'csv'
        ? generateCSV(mockData)
  ...
  const getDataStatistics = () => {
    const stats = {
      ...
      dateRange: {
        oldest: financeStore.records?.reduce((oldest, record) => {
          const recordDate = new Date(record.Data || '')
          const oldestDate = new Date(oldest)
          return recordDate < oldestDate ? record.Data : oldest
        }, new Date().toISOString().split('T')[0]) || '',
        newest: financeStore.records?.reduce((newest, record) => {
          const recordDate = new Date(record.Data || '')
