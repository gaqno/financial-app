...
export function useSettings() {
  ...
  // Computed
  const currentLanguage = computed(() => languageOptions.find((lang) => lang.code === settings.value.language));

  const currentCurrency = computed(() => currencyOptions.find((curr) => curr.code === settings.value.currency));

  const currentTheme = computed(() => themeOptions.find((theme) => theme.value === settings.value.theme));
