...
export function useAuth() {
  ...
  const { user, session, isLoading, error, isAuthenticated, authStatus } = useAuthState();

  // Operações de autenticação básica
  const { login, register, logout, forceLogout, clearError } = useAuthActions();
  ...
  const { initializeAuth, cleanup } = useAuthCore();
