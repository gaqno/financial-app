...
export const createAuthLogger = (prefix: string) => ({
  success: (message: string, data?: any) => console.log(`✅ [${prefix}] ${message}`, data || ''),

  error: (message: string, error?: any) => console.error(`❌ [${prefix}] ${message}`, error || ''),

  info: (message: string, data?: any) => console.log(`🔍 [${prefix}] ${message}`, data || ''),

  warn: (message: string, data?: any) => console.warn(`⚠️ [${prefix}] ${message}`, data || ''),
});
