...
const validCategories = [
  'Alimentação', 'Transporte', 'Saúde', 'Educação', 'Lazer', 'Moradia',
  'Roupas', 'Tecnologia', 'Investimentos', 'Salário', 'Freelance',
  'Vendas', 'Outros', 'Família', 'Pets', 'Viagem', 'Academia', 'Beleza',
  'Presentes', 'Doações', 'Impostos', 'Seguros', 'Combustível',
  'Manutenção', 'Supermercado', 'Farmácia', 'Streaming', 'Internet'
] as const
...
export const transactionSchema = z.object({
  ...
  Categoria: z.string()
    ...
    .optional()
    .or(z.literal(''))
    .refine(cat => !cat || validCategories.includes(cat as any) || cat.length <= 50,
...
export const recurrenceSchema = z.object({
  ...
  frequency: z.enum(['semanal', 'mensal', 'trimestral', 'semestral', 'anual'], {
...
export const editTransactionSchema = transactionSchema.extend({
  ...
  // Saldo é calculado, não editável
...
export const transformTransactionData = (data: TransactionFormData) => ({
  ...
  Saldo: 0 // Será calculado posteriormente
})
