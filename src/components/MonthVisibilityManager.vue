  ...
  >
    ...
    >
      <div class="flex items-center space-x-3">
        ...
        <h3 class="text-lg font-semibold text-gray-800 dark:text-slate-100">Gerenciar Visibilidade dos Meses</h3>
    ...
    <div v-if="isExpanded" class="border-t border-gray-200 dark:border-slate-600">
      <!-- Pagination controls -->
      <div
        v-if="totalMonthPages > 1"
        class="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-700 border-b border-gray-200 dark:border-slate-600"
      >
        <button
          @click="goToPreviousMonthPage"
          :disabled="!canGoToPreviousMonthPage"
        ...
        <div class="flex items-center space-x-2">
          ...
          <div class="flex space-x-1">
            <button
              v-for="page in Math.min(totalMonthPages, 5)"
              :key="page"
              @click="goToMonthPage(page - 1)"
              :class="[
        ...
        <button
          @click="goToNextMonthPage"
          :disabled="!canGoToNextMonthPage"
          class="flex items-center space-x-2 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-md hover:bg-gray-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          <span>Próximos</span>
        ...
        >
          <div class="flex items-center space-x-3">
            ...
            <span class="text-sm text-gray-500">
              ({{ month.recordCount }} registro{{ month.recordCount !== 1 ? 's' : '' }})
            </span>
          </div>
          <div class="flex items-center space-x-2">
            ...
            <button
              ...
              class="flex items-center space-x-1 px-3 py-1.5 text-sm font-medium rounded-md transition-colors"
              :class="
                month.isHidden
              ...
              "
            >
              <i :class="month.isHidden ? 'fas fa-eye' : 'fas fa-eye-slash'" class="text-xs" />
      ...
      >
        ...
        <div class="flex items-center space-x-2">
          ...
          >
            <i class="fas fa-eye-slash text-xs" />
...
<script setup lang="ts">
  ...
  const visibleMonthsCount = computed(() => {
    return (
      props.totalAvailableMonths -
      Object.values(props.paginatedAvailableMonths).filter((month) => month.isHidden).length
    );
  });
