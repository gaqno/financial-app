      ...
      <div class="p-4 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-3">
            ...
            <div class="text-lg font-bold text-blue-900">{{ stateSnapshot.dataCount || 0 }}</div>
          ...
          <div class="bg-orange-50 border border-orange-200 rounded-lg p-3">
            ...
            <div class="text-lg font-bold text-orange-900">{{ stateSnapshot.hiddenMonthsCount || 0 }}</div>
            <div class="text-xs text-orange-600">{{ (stateSnapshot.hiddenMonths || []).join(', ') || 'nenhum' }}</div>
          </div>

          <div class="bg-green-50 border border-green-200 rounded-lg p-3">
            <div class="text-sm font-medium text-green-800">Filtros Ativos</div>
            <div class="text-sm font-bold text-green-900">{{ stateSnapshot.currentFilter || 'all' }}</div>
            <div class="text-xs text-green-600">{{ stateSnapshot.categoryFilter || 'todas categorias' }}</div>
          </div>

          <div class="bg-purple-50 border border-purple-200 rounded-lg p-3">
            <div class="text-sm font-medium text-purple-800">Recorrência</div>
            <div class="text-sm font-bold text-purple-900">{{ stateSnapshot.isRecurring ? 'Ativo' : 'Inativo' }}</div>
            <div class="text-xs text-purple-600">{{ stateSnapshot.recurrenceSettings?.frequency || 'mensal' }}</div>
          </div>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4">
          ...
          <div class="space-y-2">
            <div v-for="(value, key) in localStorageState" :key="key"
              ...
              <button @click="clearSpecificKey(key)"
        ...
        <div class="bg-gray-50 rounded-lg p-4">
          ...
          <textarea v-model="jsonState"
            class="w-full h-32 text-xs font-mono bg-white border border-gray-300 rounded p-2 resize-none" readonly
            placeholder="Estado em formato JSON..."></textarea>
        </div>
      </div>
    </div>
  </section>
