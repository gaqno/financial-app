<template>
  <div class="relative">
    ...
    <div v-else class="space-y-2">
      ...
      <div class="text-xs text-gray-600" :class="previewClass">📅 {{ formatDate(modelValue) }}</div>
    </div>
    <button
      type="button"
...
<script setup lang="ts">
  ...
  const props = withDefaults(defineProps<Props>(), {
    ...
    selectClass:
      'w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent',
    toggleButtonClass:
      'absolute top-1 right-1 w-6 h-6 bg-blue-500 hover:bg-blue-600 text-white rounded-full text-xs flex items-center justify-center',
