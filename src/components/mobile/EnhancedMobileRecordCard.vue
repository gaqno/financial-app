<template>
  <div ref="cardRef" @touchstart="handleTouchStart" @touchmove="handleTouchMove" @touchend="handleTouchEnd"
    :data-record-id="record.id" :style="{ transform: `translateX(${translateX}px)` }"
    class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:shadow-[0_2px_8px_rgba(0,0,0,0.2)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] dark:hover:shadow-[0_4px_16px_rgba(0,0,0,0.3)] mb-4 overflow-hidden transition-all duration-200 theme-transition"
    :class="[
      getCardClasses(),
      {
    ...
    <!-- Card Content -->
    <div class="relative z-10 p-4 bg-white dark:bg-slate-800 transition-colors duration-200">
      ...
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center space-x-3">
          ...
          ]" style="box-shadow: 0 0 0 3px var(--tw-shadow-color);"></div>
        ...
        <div class="flex items-center space-x-1">
          <button @click.stop="handleEdit"
            class="p-2 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-slate-700"
        ...
        <div class="flex items-start justify-between mb-2">
          <div class="flex-1 mr-3">
            ...
            <div v-if="record.Categoria" class="flex items-center mt-1">
              <i :class="getCategoryIcon(record.Categoria)" class="mr-2 text-gray-500 dark:text-slate-400 text-sm"></i>
              <span class="text-sm text-gray-600 dark:text-slate-400">{{ record.Categoria }}</span>
            </div>
          </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <!-- Status Button -->
        <button @click.stop="handleStatusToggle"
      ...
      <Transition name="details">
        <div v-if="showDetails"
          class="mt-4 pt-4 relative before:absolute before:top-0 before:left-1/2 before:-translate-x-1/2 before:w-12 before:h-0.5 before:bg-gradient-to-r before:from-transparent before:via-gray-300 before:to-transparent dark:before:via-slate-500">
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              <div class="text-gray-500 dark:text-slate-400 mb-1">Data Completa</div>
              <div class="font-medium text-gray-900 dark:text-slate-100">{{ formatDate(record.Data) }}</div>
            </div>
            <div>
              <div class="text-gray-500 dark:text-slate-400 mb-1">Valor Absoluto</div>
              <div class="font-medium text-gray-900 dark:text-slate-100">{{ formatCurrency(Math.abs(record.Valor)) }}
              </div>
            </div>
            <div v-if="record.recurrence" class="col-span-2">
              <div class="text-gray-500 dark:text-slate-400 mb-1">Recorrência</div>
              <div class="font-medium text-gray-900 dark:text-slate-100">
                A cada {{ record.recurrence.intervalValue }} {{ getIntervalLabel(record.recurrence.intervalType) }}
              </div>
            </div>
          </div>
        </div>
      ...
      <button @click.stop="showDetails = !showDetails"
        class="w-full mt-3 py-2 text-center text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300 transition-colors">
        <i :class="showDetails ? 'fas fa-chevron-up' : 'fas fa-chevron-down'" class="text-xs"></i>
      </button>
    </div>

    <!-- Swipe Hint (First time users) -->
    <div v-if="showSwipeHint && !hasUserSwiped"
      class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-20 rounded-2xl">
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 m-4 text-center">
        ...
        <button @click="dismissSwipeHint" class="text-blue-500 text-sm font-medium">
          Entendi
        </button>
      </div>
    </div>
...
const getCardClasses = () => {
  ...
  if (props.record.Status === '✔️') {
    baseClasses.push('bg-gray-50/70 dark:bg-slate-700/70', 'border-l-4', 'border-l-green-400')
  }
...
const getTypeClasses = () => {
  const baseClasses = props.record.Tipo === 'Receita'
...
const getStatusText = () => {
  switch (props.record.Status) {
    case '✔️': return 'Concluído'
    case '⏰': return 'Agendado'
