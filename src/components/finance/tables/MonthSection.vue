<template>
  <div class="month-section">
    ...
    <div @click="toggleCollapse"
      class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4 mb-2 cursor-pointer hover:from-blue-100 hover:to-indigo-100 transition-all duration-200">
      ...
      <div v-if="!isCollapsed" class="mt-3">
        <div class="flex items-center justify-between text-xs text-gray-600 mb-1">
          <div class="flex items-center gap-2">
            ...
            <span class="text-gray-400">{{ completedCount }}/{{ recordsCount }}</span>
          </div>
          <div v-if="pendingCount > 0" class="flex items-center gap-1 text-orange-600">
            <i class="fas fa-clock text-xs"></i>
            <span class="font-medium">{{ formatCurrency(pendingAmount) }} pendente</span>
            <span class="text-gray-400">({{ pendingCount }})</span>
          </div>
        </div>
      ...
      <!-- Category Summary -->
      <div v-if="!isCollapsed && categorySummary.length > 0" class="mt-4 pt-3 border-t border-blue-200">
        <div class="flex items-center gap-2 text-xs text-blue-700 mb-2">
          <i class="fas fa-chart-pie"></i>
          <span>Resumo por Categoria</span>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          <div v-for="category in categorySummary" :key="category.name" class="bg-white/60 rounded-md p-2 text-xs">
            <div class="flex items-center gap-1 mb-1">
              <span>{{ category.icon }}</span>
              <span class="font-medium text-gray-700 truncate">{{ category.name }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-gray-500">{{ category.count }}x</span>
    ...
    <Transition name="month-content" enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="!isCollapsed" class="month-content">
        <!-- Desktop Table -->
        <div class="hidden lg:block bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
        ...
        <div class="lg:hidden space-y-3 mb-6">
          <div v-for="(record, index) in records"
            ...
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between items-center">
                <span class="text-gray-600">{{ record.Categoria || 'Sem categoria' }}</span>
                <button @click="handleToggleStatus(record, getGlobalIndex(index))" class="px-2 py-1 rounded text-xs"
                  :class="record.Status === '✔️' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'">
