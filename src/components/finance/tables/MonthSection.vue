<template>
  <div class="month-section">
    ...
    <div @click="toggleCollapse"
      class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900 dark:to-indigo-900 border border-blue-200 dark:border-blue-700 rounded-lg p-4 mb-2 cursor-pointer hover:from-blue-100 hover:to-indigo-100 dark:hover:from-blue-800 dark:hover:to-indigo-800 transition-all duration-200">
      <div class="flex items-center justify-between">
        ...
        <div class="text-right">
          ...
          <div class="text-xs text-gray-600 dark:text-slate-400 space-x-2">
      ...
      <div v-if="!isCollapsed" class="mt-3">
        <div class="flex items-center justify-between text-xs text-gray-600 dark:text-slate-400 mb-1">
          <div class="flex items-center gap-2">
            <span>Progresso do mês</span>
            <span class="text-gray-400 dark:text-slate-500">{{ completedCount }}/{{ recordsCount }}</span>
          </div>
          <div v-if="pendingCount > 0" class="flex items-center gap-1 text-orange-600 dark:text-orange-400">
            ...
            <span class="text-gray-400 dark:text-slate-500">({{ pendingCount }})</span>
          </div>
        </div>
      ...
      <!-- Category Summary -->
      <div v-if="!isCollapsed && categorySummary.length > 0" class="mt-4 pt-3 border-t border-blue-200 dark:border-blue-700">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          <div v-for="category in categorySummary" :key="category.name" class="bg-white/60 dark:bg-slate-700/60 rounded-md p-2 text-xs">
            <div class="flex items-center gap-1 mb-1">
              <span>{{ category.icon }}</span>
              <span class="font-medium text-gray-700 dark:text-slate-300 truncate">{{ category.name }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-gray-500 dark:text-slate-400">{{ category.count }}x</span>
    ...
    <Transition name="month-content" enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="!isCollapsed" class="month-content">
        <!-- Desktop Table -->
        <div class="hidden lg:block bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-600 overflow-hidden mb-6 theme-transition">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-600">
            <thead class="bg-gray-50 dark:bg-slate-700">
              <tr>
                <th @click="handleSort('Data')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors">
                  <div class="flex items-center gap-1">
                    <span>Data</span>
                    <i :class="getSortIcon('Data')" class="text-xs"></i>
                  </div>
                </th>
                <th @click="handleSort('Descrição')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors">
                  <div class="flex items-center gap-1">
                    <span>Descrição</span>
                    <i :class="getSortIcon('Descrição')" class="text-xs"></i>
                  </div>
                </th>
                <th @click="handleSort('Valor')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors">
                  <div class="flex items-center gap-1">
                    <span>Valor</span>
                    <i :class="getSortIcon('Valor')" class="text-xs"></i>
                  </div>
                </th>
                <th @click="handleSort('Tipo')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors">
                  <div class="flex items-center gap-1">
                    <span>Tipo</span>
                    <i :class="getSortIcon('Tipo')" class="text-xs"></i>
                  </div>
                </th>
                <th @click="handleSort('Categoria')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors">
                  <div class="flex items-center gap-1">
                    <span>Categoria</span>
                    <i :class="getSortIcon('Categoria')" class="text-xs"></i>
                  </div>
                </th>
                <th @click="handleSort('Status')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors">
                  <div class="flex items-center gap-1">
                    <span>Status</span>
                    <i :class="getSortIcon('Status')" class="text-xs"></i>
                  </div>
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Saldo</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Ações</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-600">
              <RecordRow v-for="(record, index) in records"
                :key="`${record.Data}-${record.Descrição}-${record.Status}-${index}`" :record="record"
                :index="getGlobalIndex(index)" @edit="(record, globalIndex) => handleEdit(record, globalIndex)"
        ...
        <div class="lg:hidden space-y-3 mb-6">
          <div v-for="(record, index) in records"
            ...
            class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-600 p-4 theme-transition">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between items-start">
                <span class="font-medium text-gray-900 dark:text-slate-100">{{ record.Descrição }}</span>
                <div class="flex gap-2">
                  <button @click="handleEdit(record, getGlobalIndex(index))"
                    class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 p-1">
                    <i class="fas fa-edit"></i>
                  </button>
                  <button @click="handleDelete(record, getGlobalIndex(index))"
              ...
              <div class="flex justify-between items-center">
                <span class="text-gray-600 dark:text-slate-300">{{ record.Categoria || 'Sem categoria' }}</span>
                <button @click="handleToggleStatus(record, getGlobalIndex(index))" class="px-2 py-1 rounded text-xs"
                  :class="record.Status === '✔️' ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-300' : 'bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-300'">
...
const completedCount = computed(() =>
  props.records.filter(record => record.Status === '✔️').length
)
...
const pendingCount = computed(() =>
  props.records.filter(record => record.Status === '❌').length
)
...
const monthSummary = computed(() => {
  const receitas = props.records
    .filter(r => r.Tipo === 'Receita')
    .reduce((sum, r) => sum + r.Valor, 0)

  const despesas = props.records
...
const totalBalanceColor = computed(() =>
  monthSummary.value.saldo < 0 ? 'text-red-600' : 'text-green-600'
)
