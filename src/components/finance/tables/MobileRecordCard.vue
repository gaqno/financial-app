<template>
  <div
    class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-600 p-4 transition-all duration-200 theme-transition"
  ...
  >
    ...
    <div class="flex items-center justify-between mb-3">
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          :class="{ 'opacity-60': record.Status === '✔️' }"
    ...
    <div class="flex items-center justify-between">
      <button
        ...
        :title="getStatusToggleTitle(record.Status)"
      >
        {{ record.Status }}
...
<script setup lang="ts">
  ...
  const getCardClasses = () => {
    ...
    if (props.record.Status === '✔️') {
      ...
      baseClasses.push('bg-gray-50/70', 'border-l-4', 'border-l-gray-300');
    }
  ...
  const getTextClasses = () => {
    return props.record.Status === '✔️' ? 'text-gray-600' : 'text-gray-900';
  };
  ...
  const getTypeClasses = (tipo: 'Receita' | 'Despesa') => {
    const baseClasses = tipo === 'Receita' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
