<template>
  ...
  >
    ...
    <div
      ...
      class="bg-gradient-to-r rounded-md mb-2 from-purple-50 to-indigo-50 dark:from-purple-900 dark:to-indigo-900 border-b border-purple-200 dark:border-purple-700 p-4 cursor-pointer hover:from-purple-100 hover:to-indigo-100 dark:hover:from-purple-800 dark:hover:to-indigo-800 transition-all duration-200 select-none"
      role="button"
    ...
    >
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex items-center gap-3 flex-1 min-w-0">
          <button
            class="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 transition-colors flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-purple-400"
            tabindex="-1"
            aria-label="Expandir/Recolher projeção anual"
            @click.stop="toggleCollapse"
          >
            <i
              :class="isCollapsed ? 'fas fa-chevron-right' : 'fas fa-chevron-down'"
              class="text-base sm:text-sm transition-transform duration-200"
            ></i>
          </button>
          <div class="min-w-0">
            <h2 class="text-base sm:text-lg font-semibold text-purple-900 dark:text-purple-200 truncate">
              <i class="fas fa-chart-line mr-2"></i>
              Projeção Anual {{ currentYear }}
            </h2>
            <p class="text-xs sm:text-sm text-purple-700 dark:text-purple-300 truncate">
        ...
        <div class="text-right flex-shrink-0 min-w-[110px]">
          <div class="text-base sm:text-lg font-bold" :class="projectedBalance < 0 ? 'text-red-600' : 'text-green-600'">
            {{ formatCurrency(projectedBalance) }}
          </div>
          <div class="text-xs text-purple-700 dark:text-purple-300 whitespace-nowrap">Projeção para dezembro</div>
        </div>
      </div>
    </div>
    ...
    >
      <div v-if="!isCollapsed" class="space-y-6">
        <!-- Monthly Averages -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="bg-green-50 dark:bg-green-900 rounded-lg p-4 border border-green-200 dark:border-green-700">
            ...
            <div class="text-xs text-green-600 dark:text-green-400 mt-1">por mês</div>
          </div>
          ...
          <div class="bg-red-50 dark:bg-red-900 rounded-lg p-4 border border-red-200 dark:border-red-700">
            ...
            <div class="text-xs text-red-600 dark:text-red-400 mt-1">por mês</div>
          </div>
          ...
          <div class="bg-blue-50 dark:bg-blue-900 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
            ...
            <div
              ...
              :class="averageBalance < 0 ? 'text-red-700 dark:text-red-300' : 'text-green-700 dark:text-green-300'"
            >
              {{ formatCurrency(averageBalance) }}
            </div>
            <div class="text-xs text-blue-600 dark:text-blue-400 mt-1">por mês</div>
          </div>
        </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-600">
                <th class="text-left py-2 px-3 text-sm font-medium text-gray-600 dark:text-gray-400">Mês</th>
                <th class="text-right py-2 px-3 text-sm font-medium text-gray-600 dark:text-gray-400">
                  Receitas Proj.
                </th>
                <th class="text-right py-2 px-3 text-sm font-medium text-gray-600 dark:text-gray-400">
                  Despesas Proj.
                </th>
                <th class="text-right py-2 px-3 text-sm font-medium text-gray-600 dark:text-gray-400">Saldo Mensal</th>
                <th class="text-right py-2 px-3 text-sm font-medium text-gray-600 dark:text-gray-400">
                  Saldo Acumulado
                </th>
              </tr>
            ...
            <tbody>
              <tr
                v-for="month in yearlyProjection"
                :key="month.month"
                :class="[
                  month.isActual ? 'bg-gray-50 dark:bg-gray-700' : 'bg-purple-50/30 dark:bg-purple-900/30',
              ...
              >
                <td class="py-2 px-3 text-sm">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      v-if="month.isActual"
                      class="text-xs bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 px-1 rounded"
                    >
                      Real
                    </span>
                    <span
                      v-else
                      class="text-xs bg-purple-200 dark:bg-purple-700 text-purple-700 dark:text-purple-300 px-1 rounded"
                    >
                      Projeção
                    </span>
                  </div>
                ...
                <td
                  class="py-2 px-3 text-sm text-right font-semibold"
                  :class="month.balance < 0 ? 'text-red-600' : 'text-green-600'"
                >
                  {{ formatCurrency(month.balance) }}
                </td>
                <td
                  class="py-2 px-3 text-sm text-right font-bold"
        ...
        <div v-if="categoryProjections.length > 0">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <div
              v-for="category in categoryProjections"
              :key="category.name"
            ...
            >
              ...
              <div class="space-y-1">
                <div class="flex justify-between text-sm">
                  <span class="text-gray-600 dark:text-gray-400">Mensal médio:</span>
                ...
                <div class="flex justify-between text-sm">
                  <span class="text-gray-600 dark:text-gray-400">Projeção anual:</span>
    ...
    return Array.from(groups.entries())
      .map(([monthKey, records]) => {
        ...
        return {
          ...
          year,
          month,
          monthName: new Date(year, month - 1).toLocaleDateString('pt-BR', {
            month: 'long',
            year: 'numeric',
          }),
          income,
  ...
  const yearlyProjection = computed(() => {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      if (actualData) {
        ...
        months.push({
          month,
          monthName: new Date(currentYear, month - 1).toLocaleDateString('pt-BR', {
            month: 'long',
            year: 'numeric',
          }),
