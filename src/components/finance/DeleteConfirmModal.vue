<template>
  <Transition name="modal">
    <div v-if="financeStore.showDeleteConfirm" data-modal="delete-confirm"
        ...
        <div class="text-center">
          ...
          <div v-if="financeStore.itemToDelete" class="bg-gray-50 rounded-lg p-3 mb-4 text-left">
            <div class="text-sm text-gray-800 font-medium flex items-center gap-1">
              ...
              <span v-if="isRecurringRecord(financeStore.itemToDelete.record)" class="text-blue-500 text-xs"
                title="Registro recorrente">
                <i class="fas fa-sync"></i>
              </span>
            </div>
            <div class="text-sm text-gray-600">{{ formatDate(financeStore.itemToDelete.record.Data) }}</div>
            <div :class="financeStore.itemToDelete.record.Valor < 0 ? 'text-red-600' : 'text-green-600'"
              class="text-sm font-semibold">
              {{ financeStore.itemToDelete.record.Valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
              }}
            </div>
          </div>

          <div class="flex gap-3">
            <button @click="financeStore.cancelDelete"
