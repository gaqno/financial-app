  ...
  <section class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="p-4 border-b border-gray-200">
      <div class="flex flex-wrap gap-2">
        <button @click="setFilter('all')" :class="getFilterButtonClass('all')"
          class="px-4 py-2 rounded-full text-sm font-medium transition-colors">
          📊 Todos
        </button>
        <button @click="setFilter('Receita')" :class="getFilterButtonClass('Receita')"
          class="px-4 py-2 rounded-full text-sm font-medium transition-colors">
          💰 Receitas
        </button>
        <button @click="setFilter('Despesa')" :class="getFilterButtonClass('Despesa')"
          class="px-4 py-2 rounded-full text-sm font-medium transition-colors">
          💸 Despesas
        </button>
      </div>
    ...
    <div v-if="showAdvanced && showAdvancedFilters" class="p-4 bg-gray-50">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input v-model="searchTerm" type="text" placeholder="Descrição ou categoria..."
        ...
        <div>
          ...
          <div class="flex gap-1">
            <button @click="showRecentTransactions(7)"
              class="flex-1 px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded">
              7 dias
            </button>
            <button @click="showRecentTransactions(30)"
              class="flex-1 px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded">
              30 dias
            </button>
      ...
      <div v-if="hasActiveFilters" class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div class="flex items-center justify-between">
          <div class="text-sm text-blue-800">
            <strong>{{ activeFiltersCount }}</strong> filtro{{ activeFiltersCount > 1 ? 's' : '' }} ativo{{
