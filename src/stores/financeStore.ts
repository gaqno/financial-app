...
export const useFinanceStore = defineStore('finance', () => {
  ...
  const sortedData = computed(() => {
    ...
    return smartVisibleRecords.sort((a, b) => {
  ...
  const saldoPendente = computed(() => {
    return Math.abs(
      filteredData.value
        ...
        .reduce((total, item) => total + item.Valor, 0)
    ); // Math.abs to get positive value
  });
  ...
  const updateRecord = async (index: number, updatedData: Partial<IFinanceRecord>): Promise<boolean> => {
    try {
      ...
      console.log('🏪 [STORE_UPDATE] Updating record:', {
        index,
        currentRecord: {
          desc: currentRecord.Descrição,
          date: currentRecord.Data,
          status: currentRecord.Status,
          hasRecurrence: !!currentRecord.recurrence,
        },
        updatedData: {
          desc: updatedData.Descrição,
    ...
    try {
      ...
      if (isRecurringRecord) {
        ...
        if (shouldUpdateAll) {
          ...
          const batchUpdateSuccess = await updateAllLinkedRecurringRecords(
            originalRecord,
            {
            ...
            if (originalRecord.recurrence?.recurrenceId) {
              try {
                ...
                if (generatedCount > 0) {
                  ...
                  if (typeof window !== 'undefined' && window.location.hostname === 'localhost') {
                    console.log(
                      `ℹ️ [EDIT] Adicionados ${generatedCount} registros futuros faltantes para completar a recorrência`
                    );
                  }
                }
              } catch (error) {
